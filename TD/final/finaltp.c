...
void sauvegarderEtudiantsEtNotes() {
    ...
    for (int i = 0; i < le.sv; i++) {
        // Vérifier si l'étudiant existe déjà dans le fichier
        bool etudiantExiste = false;
        for (int j = 0; j < le.sv; j++) {
            if (i != j && le.te[i].mat == le.te[j].mat) {
                etudiantExiste = true;
                break;
            }
        }
        ...
        if (etudiantExiste) {
            continue; // L'étudiant existe déjà, passer au suivant
        }
...
void chargerEtudiantsEtNotes() {
    ...
    while (fscanf(file, "%d,%[^,],%[^,],%d/%d/%d", &le.te[le.sv].mat, le.te[le.sv].nom, le.te[le.sv].prenom, &le.te[le.sv].dn.j, &le.te[le.sv].dn.m, &le.te[le.sv].dn.a) != EOF) {
        ...
        for (int j = 0; j < lu.lib; j++) {
            fscanf(file, ",%.1f", &le.te[le.sv].sn.tu[j].n);
        }

        le.sv++; // Incrémenter le nombre d'étudiants

        // Avancer le curseur de fichier jusqu'à la fin de la ligne
        while (fgetc(file) != '\n' && !feof(file)) {
        }
    }
